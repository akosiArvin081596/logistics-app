...
const VENDOR_ALIASES = [
	...
	["LOVE'S", /LOVE'?S\b/],
	["SPEEDCO", /SPEEDCO/],
	// PETRO\b matches the TA/Petro truck-stop brand, but NOT "Petro-Canada"
	// (a different, Canadian fuel retailer) — the negative lookahead excludes it.
	["TA PETRO", /TRAVEL\s*CENTERS?\s+OF\s+AMERICA|TA\s+EXPRESS|PETRO\b(?![\s-]*CANADA)/],
	["SPEEDWAY", /SPEEDWAY/],
	["CASEY'S", /CASEY'?S\b/],
...
// filters = { from, to, types, states, driver, truck, q, vendor, vendorLike }
// — all optional, pre-validated by callers. Returns { where, params } where
...
function buildExpenseFilterSql(filters = {}) {
	...
	if (f.vendorLike) {
		// Vendor-scoped substring match (AI "vendorContains") — deliberately
		// narrower than `q`: matching description/city here would silently pull
		// in every row from a same-named CITY (e.g. "Dallas").
		const esc = String(f.vendorLike).replace(/[\\%_]/g, (c) => "\\" + c);
		const like = `%${esc}%`;
		conds.push("(vendor LIKE ? ESCAPE '\\' OR vendor_normalized LIKE ? ESCAPE '\\')");
...
// Types where a merchant name is genuinely expected — drives the "missing
// vendor" data-quality hint. Toll / Food / Other (trip & IFTA permits, misc
// fees) legitimately have no store, so a blank vendor on those is not flagged.
const VENDOR_EXPECTED_TYPES = new Set(["Fuel", "Repair", "Maintenance", "Wear & Tear"]);
...
function aggregateExpenses(db, filters = {}) {
	...
	for (const r of rows) {
		...
		} else if (VENDOR_EXPECTED_TYPES.has(String(r.type || ""))) {
			// Blank vendor only counts as a gap on fuel/repair-type rows — a
			// permit or toll with no store name is expected, not a data gap.
			unknownVendorCount++;
...
function runQuerySpec(db, spec) {
	...
	const rows = fetchExpenseRows(db, {
		...
		vendorLike: f.vendorContains || undefined,
	});
	...
	// The requested metric must appear in the table — the answer model is
	// forbidden from citing numbers that aren't in it (state/driver/truck/type
	// groups don't carry avgPerGallon by default).
	if (metric === "avg_per_gallon" && !columnKeys.includes("avgPerGallon")) columnKeys.push("avgPerGallon");
