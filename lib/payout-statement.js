...
function buildPayoutStatementHtml(fields = {}) {
	...
	// THE DOCUMENT DATES ITSELF, BECAUSE IT IS NOW CACHED.
	// Owner-approved 2026-08-17; recorded in docs/investor-portal-copy.md §12.
	//
	// Rendered statements are stored and re-served (investor_payouts
	// .statement_pdf_file_name in server.js), so a PDF is a snapshot of the moment
	// it was first built, not a live view. Two parts of this page are a LIVE
	// recompute at render time — the itemized appendix, and page 1's composition
	// on a period with no finalized_breakdown snapshot — so a statement built
	// while everything agreed is frozen that way and will never later acquire
	// driftNote()'s "now computes to" caveat, however far the portal moves.
	//
	// Rather than defeat the cache to keep chasing the portal, the document says
	// which it is. Printed in BOTH modes deliberately: the freeze applies to a
	// FINAL statement exactly as it does to a PAID one.
	//
	// ⚠️ Reuses `generatedAt` — the same value, through the same formatDate(),
	// that the header prints as "Issued". Never re-derive a date here: two date
	// formats on one money document is its own bug.
	//
	// ⚠️ Built here rather than inline in the HTML below. That markup is one big
	// template literal, so a backtick or a `${` inside an HTML comment there
	// terminates the string — which is exactly how the first draft of this failed
	// `node --check`.
	const asOfLine = `Figures as of ${generatedAt}; the portal shows current records.`;
...
/* The "figures as of" caveat. Nested INSIDE .paidmeta so it hugs the line above
   it rather than sitting 26px adrift, and deliberately a step smaller and
   lighter than .paidmeta (which is itself a step below the amount pill): it is
   a caveat, not a headline, and must never compete with the settled figure. */
.asof { font-size: 9.5px; color: #94a3b8; font-style: italic; margin-top: 4px; }
...
<div class="doc">
	...
	<div class="paidmeta">${
		...
		<!-- The document dates itself because it is CACHED - see asOfLine above. -->
		<div class="asof">${asOfLine}</div></div>
