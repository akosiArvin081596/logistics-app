...
function buildPayoutStatementHtml(fields = {}) {
	...
	if (b) {
		...
		waterfallRows += `
			...
			<tr><td class="ded">&minus; Trip Expenses <span class="tag">your expenses</span></td><td class="num ded">${esc(negMoney(b.tripExpenses))}</td></tr>`;
	...
	waterfallRows += `
			<tr class="rule"><td class="strong">Settled amount for ${periodLabel}${
				drifted
					? `<div class="cap">Recorded on the ledger when this period was settled. The composition above reflects our records as of ${generatedAt} and now computes to ${esc(money(composed))}; the settled amount is what was paid.</div>`
					: ""
...
.tag { display: inline-block; margin-left: 6px; padding: 1px 7px; border-radius: 999px; background: #fffbeb; border: 1px solid #fde68a; color: #92400e; font-size: 9px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; }
...
<div class="doc">
	...
	<div class="band">
		...
		<div><span class="k">Status</span><span class="stamp">${status}</span></div>
		${paidAt ? `<div><span class="k">Paid on</span><span class="v">${paidAt}</span></div>` : ""}
	</div>
	...
	<div class="totals">
		<div class="box">
			<div class="pill"><span class="lbl">${adjustedAfterPaid ? "Net Settled" : "Amount Paid"}</span><span class="amt">${esc(money(effectiveAmount))}</span></div>
		</div>
	</div>
	<div class="paidmeta">${
		adjustedAfterPaid
			? `${esc(money(amount))} paid on ${paidAt}${adjustedAt ? ` &middot; correction recorded ${adjustedAt}` : ""} &middot; net ${esc(money(effectiveAmount))}`
			: `${paidAt ? `Paid on ${paidAt}` : "Settled"}${paidBy ? ` &middot; recorded by ${paidBy}` : ""}`
	}</div>
