...
function generateSubstancePolicy(data) {
	return new Promise((resolve, reject) => {
		...
		// Designee table
		const tblX = MARGIN + 100;
		const tblY = doc.y;
		doc.rect(tblX, tblY, 300, 60).stroke();
		doc.font(B).fontSize(fs).text("NAME:", tblX + 10, tblY + 8);
		doc.font(R).text("Leslie M. Johnson", tblX + 90, tblY + 8);
		doc.font(B).text("LOCATION:", tblX + 10, tblY + 24);
		doc.font(R).text("Conroe, TX 77301", tblX + 90, tblY + 24);
		doc.font(B).text("PHONE:", tblX + 10, tblY + 40);
		doc.font(R).text("321-848-3437", tblX + 90, tblY + 40);
		doc.y = tblY + 70;
		...
		t1.forEach(([code, text]) => {
			...
			doc.font(B).fontSize(fs).text(code, MARGIN + 20, doc.y, { width: 60, continued: true });
			doc.font(R).text(" " + text, { width: W - 80, lineGap: 1.5 });
			doc.moveDown(0.2);
		});
		...
		t2rows.forEach(row => {
			...
			doc.font(B).text(row[1], MARGIN + 20 + t2cols[0], rowY, { width: t2cols[1] - 10 });
			rowY += 18;
		});
		doc.y = rowY + 10;
