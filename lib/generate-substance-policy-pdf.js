...
function generateSubstancePolicy(data) {
	return new Promise((resolve, reject) => {
		...
		// Designee table (bordered, matching original template)
		doc.moveDown(0.5);
		const tblX = MARGIN + 80;
		const labelW = 90;
		const valW = 250;
		const tblW = labelW + valW;
		const rowH = 22;
		const tblY = doc.y;
		const designeeRows = [
			["NAME:", "Leslie M. Johnson"],
			["LOCATION:", "Conroe, TX 77301"],
			["PHONE:", "321-848-3437"],
		];
		designeeRows.forEach((row, i) => {
			const y = tblY + i * rowH;
			// Row border
			doc.rect(tblX, y, tblW, rowH).stroke();
			// Label cell border
			doc.moveTo(tblX + labelW, y).lineTo(tblX + labelW, y + rowH).stroke();
			// Label text (centered in cell)
			doc.font(B).fontSize(fs).fillColor(BLACK).text(row[0], tblX + 5, y + 6, { width: labelW - 10, align: "center" });
			// Value text
			doc.font(R).fontSize(fs).text(row[1], tblX + labelW + 10, y + 6, { width: valW - 15 });
		});
		doc.y = tblY + designeeRows.length * rowH + 15;
		...
		const t1CodeW = 60;
		const t1TextW = W - 20 - t1CodeW;
		t1.forEach(([code, text]) => {
			if (doc.y > 650) doc.addPage();
			const rowY = doc.y;
			const h = Math.max(doc.heightOfString(text, { width: t1TextW - 10, fontSize: fs }) + 10, 20);
			doc.rect(MARGIN + 10, rowY, t1CodeW, h).stroke();
			doc.rect(MARGIN + 10 + t1CodeW, rowY, t1TextW, h).stroke();
			doc.font(B).fontSize(fs).fillColor(BLACK).text(code, MARGIN + 15, rowY + 4, { width: t1CodeW - 10 });
			doc.font(R).fontSize(fs).text(text, MARGIN + 15 + t1CodeW, rowY + 4, { width: t1TextW - 10, lineGap: 1.5 });
			doc.y = rowY + h;
		});
		...
		t2rows.forEach(row => {
			const rY = doc.y;
			const lH = doc.heightOfString(row[0], { width: t2colW - 10, fontSize: fs }) + 10;
			const rH = doc.heightOfString(row[1], { width: t2colW - 10, fontSize: fs }) + 10;
			const h = Math.max(lH, rH);
			doc.rect(MARGIN + 10, rY, t2colW, h).stroke();
			doc.rect(MARGIN + 10 + t2colW, rY, t2colW, h).stroke();
			doc.font(B).fontSize(fs).fillColor(BLACK).text(row[0], MARGIN + 15, rY + 4, { width: t2colW - 10 });
			doc.font(B).fontSize(fs).text(row[1], MARGIN + 15 + t2colW, rY + 4, { width: t2colW - 10 });
			doc.y = rY + h;
		});
		...
		const t3TotalW = W - 20;
		t3items.forEach((text, i) => {
			if (doc.y > 660) doc.addPage();
			const rY = doc.y;
			const h = doc.heightOfString(`${i + 1}. ${text}`, { width: t3TotalW - 20, fontSize: fs }) + 8;
			doc.rect(MARGIN + 10, rY, t3TotalW, h).stroke();
			doc.font(R).fontSize(fs).fillColor(BLACK).text(`${i + 1}. ${text}`, MARGIN + 20, rY + 4, { width: t3TotalW - 20, lineGap: 1.5 });
			doc.y = rY + h;
		});
		...
		t4.forEach(text => {
			const rY = doc.y;
			const h = doc.heightOfString(text, { width: t4W - 15, fontSize: fs }) + 8;
			doc.rect(MARGIN + 10, rY, t4W, h).stroke();
			doc.font(R).fontSize(fs).fillColor(BLACK).text(text, MARGIN + 18, rY + 4, { width: t4W - 15, lineGap: 1.5 });
			doc.y = rY + h;
		});
