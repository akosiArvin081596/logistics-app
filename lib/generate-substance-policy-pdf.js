const { BLUE, BLACK, MARGIN, PAGE_W, createPdfDoc, drawBorderedTable } = require("./pdf-helpers");
...
function generateSubstancePolicy(data) {
	...
	const { fullName = "", effectiveDate = "", signatureImage } = data;
		...
		drawBorderedTable(doc, {
			rows: t1.map(([code, text]) => [{ text: code, bold: true }, text]),
			colWidths: [60, W - 80],
			fontSize: fs,
			pageBreakY: 650,
		});
		...
		drawBorderedTable(doc, {
			rows: [
				[{ text: "382.301 Pre-employment testing", bold: true }, { text: "382.307 Reasonable Suspicion testing", bold: true }],
				[{ text: "382.303 Post-Accident testing", bold: true }, { text: "382.309 Return to Duty testing", bold: true }],
				[{ text: "382.305 Random testing, per the prevailing rate as required by U.S. DOT", bold: true }, { text: "382.311 Follow-Up testing", bold: true }],
			],
			colWidths: [t2colW, t2colW],
			fontSize: fs,
		});
		...
		drawBorderedTable(doc, {
			rows: t3items.map((text, i) => [`${i + 1}. ${text}`]),
			colWidths: [W - 20],
			fontSize: fs,
		});
