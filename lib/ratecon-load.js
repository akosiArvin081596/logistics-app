...
//
// ⚠️ The old instruction here — "keep this byte-for-byte equivalent to the
// workflow so an email-ingested load and a dropped-PDF load produce identical
// Job Details rows" — is no longer achievable and should not be followed.
// n8n now derives distance from an LLM guess (the `AI Agent` node prompts
// Gemini to "calculate the driving distance", with no tool and no Maps call),
// while this function uses a real Distance Matrix response. The two ingestion
// paths therefore DO disagree on distance and rate-per-mile. Closing that gap
// means pointing n8n at the app, not degrading this side to match.
