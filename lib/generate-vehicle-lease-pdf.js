...
function generateVehicleLease(data) {
	...
	if (vehicles.length > 0) {
		const colWidths = [25, 40, 80, 70, 110, 60, 60];
		const headerRow = [
			{ text: "#", bold: true },
			{ text: "Year", bold: true },
			{ text: "Make", bold: true },
		...
		drawBorderedTable(doc, { rows: [headerRow, ...dataRows], colWidths, fontSize: 8.5, x: MARGIN });
		doc.x = MARGIN;
	} else {
	...
	if (signatureText) {
		const byW = doc.font(R).fontSize(9.5).widthOfString("By: ");
		const nameW = doc.font(B).fontSize(9.5).widthOfString(signatureText);
		doc.font(R).fontSize(9.5).text("By: ", MARGIN, sigLineY, { continued: true });
		doc.font(B).fillColor(BLUE).text(signatureText, { underline: true });
		doc.fillColor(BLACK).font(R);
		if (signatureImage) {
			try {
				const sigX = MARGIN + byW + nameW + 10;
				const sigBytes = Buffer.from(signatureImage.replace(/^data:image\/\w+;base64,/, ""), "base64");
				doc.image(sigBytes, sigX, sigLineY - 12, { width: 140, height: 40 });
			} catch { /* skip */ }
		}
	} else {
		doc.text("By: ___________________________________");
