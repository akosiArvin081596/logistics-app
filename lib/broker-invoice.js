...
// FALLBACK billing addresses — used only when the caller does NOT supply a
// per-load broker billing email (see resolveInvoiceTo). Bison keeps the
// dedicated AP inbox it has always used; every other broker falls back to the
// quickpay factoring inbox.
...
//
// EMAIL priority (client decision, billing-automation 2026-07): bill the
// broker's OWN AP/billing address, parsed from the rate-con per load — NOT a
// fixed factoring inbox. The server now passes `ctx.brokerBillingEmail`.
//   1) ctx.brokerBillingEmail — when supplied and shaped like an email
//      (trimmed, matches /.+@.+\..+/), it wins outright — even over Bison.
//   2) else the prior behavior — Bison → its dedicated AP inbox; every other
//      broker → the quickpay default.
// `brokerBillingEmail` is OPTIONAL and backward compatible: when it is absent
// (or blank/garbage) the result is byte-for-byte identical to before, so no
// existing caller changes.
//
// NAME resolution is UNCHANGED: Bison prints "Bison Transport"; everyone else
// goes through resolveBrokerName (domain map → humanized domain → contact
// name). The recipient address and the printed name are independent.
function resolveInvoiceTo(ctx = {}) {
	const bison = isBisonLoad({ email: ctx.brokerEmail });
	const billing = String(ctx.brokerBillingEmail == null ? "" : ctx.brokerBillingEmail).trim();
	const brokerBilling = /.+@.+\..+/.test(billing) ? billing : "";
	return {
		name: bison ? "Bison Transport" : resolveBrokerName(ctx),
		email: brokerBilling || (bison ? BISON_INVOICE_EMAIL : DEFAULT_INVOICE_EMAIL),
	};
}
...
async function extractRateConFields(pdfBuffer, opts = {}) {
	...
	if (Buffer.isBuffer(pdfBuffer) && pdfBuffer.length) {
		...
		if (scanned.orderNumber || !requireOrderNumber) fields = scanned;
	}
