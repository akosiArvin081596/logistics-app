...
//
// An HTTP failure is thrown by lib/gemini-errors.js's geminiFailure(), so it
// carries .status / .kind / .reason. Routes branch on .kind === "unavailable"
// (rejected key, exhausted quota → 503, the same answer as having no key) and
// leave .kind === "defect" (a request WE built badly) as a loud 502. Reading the
// BODY is what makes that possible: a revoked key returns HTTP 400 with reason
// API_KEY_INVALID, so it is indistinguishable from a malformed request on the
// status line alone.

const { geminiFailure } = require("./gemini-errors");
...
// Accepts the same data URIs the route pre-validates: jpeg/jpg/png/webp, plus
// application/pdf — plenty of fuel/toll receipts arrive as emailed PDFs, and
// Gemini reads them natively (the rate-con path already sends
// mime_type:"application/pdf"). Group 1 is the subtype, group 2 the payload.
const DATA_URI_RE = /^data:(image\/(?:jpeg|jpg|png|webp)|application\/pdf);base64,(.+)$/i;
...
- If the document is not a receipt, return every field null with confidence "low".
- details: an array of {label, value} pairs for every OTHER meaningful line printed on the receipt that isn't already captured by the fields above — e.g. fuel: pump #, fuel grade, price/gallon; food: line items, subtotal, tax, tip; tolls: plaza, axles, class; maintenance: labor, parts, invoice #; general: subtotal, tax, payment method, card last-4, auth code, store address. Keep each label <=40 chars and value <=200 chars. Include ONLY what is actually printed and legible; omit the rest. Return [] if there is nothing else.
...
function shapeResponse(parsed) {
	...
	// Dynamic label/value details — untrusted model output, so drop non-objects,
	// require a non-empty label, and cap both the item count and string lengths.
	if (Array.isArray(src.details)) {
		out.details = src.details
...
async function runReceiptOcr(imageDataUri) {
	...
	if (!m) {
		const err = new Error("receipt OCR requires a base64 image or PDF data URI");
		err.code = "OCR_BAD_INPUT";
		throw err;
	}
	// m[1] is the full type now ("image/png", "application/pdf"); only the
	// non-canonical image/jpg needs remapping.
	...
	const body = {
		...
		generationConfig: {
			...
			// this budget, so a tight cap truncated the JSON on some receipts and
			// surfaced as "Response was not valid JSON". Bumped to 4096 to leave
			// headroom for thinking + the dynamic `details` array (cost stays low).
	...
	for (let attempt = 0; attempt <= DEFAULT_RETRIES; attempt++) {
		...
		try {
			...
			if (!resp.ok) {
				// Carries .status (as before) plus .kind / .reason, read out of
				// Google's body. The message format is unchanged.
			...
			lastErr = err;
			// Only retry things that can actually succeed on a second try. A bad
			// request, a rejected key or a payload that's too large will fail
			// identically every time — retrying just multiplies the wait. This is
			// not theoretical: when Google began 403-ing an unrestricted API key,
			// every receipt sat ~45s (3 attempts x 15s) before reporting failure,
			// so a bulk batch looked frozen instead of failing fast.
			//
			// Keyed on the CLASSIFICATION rather than a status set, which is
			// strictly wider by one real case: a credential/quota reason wrapped in
			// a 5xx now stops too. Errors with no .kind (empty body, unparseable
			// JSON, an aborted fetch) still retry exactly as they did.
