...
// Statuses that will fail the same way every time: malformed request, rejected /
// restricted key, missing model, payload too large, quota exhausted. Retrying
// these only makes the caller wait 3x longer for the same answer.
	...
	for (let attempt = 0; attempt <= DEFAULT_RETRIES; attempt++) {
		...
		try {
			...
			if (!resp.ok) {
				...
				const e = new Error(`Gemini ${resp.status}: ${errText.slice(0, 200)}`);
			...
			if (NON_RETRYABLE_STATUS.has(err && err.status)) break;
