...
// Apollo's GET endpoints carry the API key as a query-string value, and its
// server does NOT url-decode query values — so a key containing any URL-reserved
// character (notably '?') can't be delivered on a GET at all: it comes back
// code:7 "invalid key" even though the key is valid. Verified 2026-07 with a
// '?'-containing key that authenticated fine in POST bodies but failed EVERY GET
// (both raw '?' and '%3F'). A clean Apollo key is [A-Za-z0-9]; flag anything
// outside the RFC3986 unreserved set as GET-unsafe so callers get a clear
// message instead of a bogus "invalid key". POST-based calls are unaffected.
function unsafeGetKeyChars(apiKey) {
	const m = String(apiKey || "").match(/[^A-Za-z0-9._~-]/g);
	return m ? Array.from(new Set(m)) : [];
}
function keyIsGetSafe(apiKey) {
	return !!apiKey && unsafeGetKeyChars(apiKey).length === 0;
}
...
async function request({ apiKey, baseUrl }, method, path, { query, body } = {}) {
	...
	// Fail fast + clearly when a URL-unsafe key is sent to a GET endpoint. Apollo
	// would otherwise answer code:7, which masquerades as an invalid key and sent
	// us chasing a non-problem. POST endpoints put the key in the body, so they
	// are exempt — the key is genuinely valid, just not GET-transmittable.
	if (isGet) {
		const bad = unsafeGetKeyChars(apiKey);
		if (bad.length) {
			const err = new Error(
				`Apollo API key contains ${bad.map((c) => `'${c}'`).join(", ")} — Apollo's GET endpoints ` +
				`(live tracking, drivers) reject keys with URL-reserved characters. Ask the reseller to ` +
				`regenerate a key with only letters and numbers.`,
			);
			err.code = "APOLLO_KEY_NOT_URL_SAFE";
			throw err;
		}
	}
...
function normalizeTelemetry(row) {
	...
	return {
		...
		location_date_ms: toEpochMs(firstDefined(r.LocationDate, r.LocationDateUtc, r.GpsTime, r.LastUpdate)), // LIVE-CAPTURE: verify field name + unit
...
module.exports = {
	...
	listDrivers,
	listHosClocks,
	// Key-safety helpers — used by the server's startup guard to warn when the
	// configured Apollo key can't be delivered on GET endpoints (see the
	// unsafeGetKeyChars comment above).
	keyIsGetSafe,
	unsafeGetKeyChars,
