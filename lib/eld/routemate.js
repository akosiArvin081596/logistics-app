// Routemate AI ELD/telematics adapter (provider "routemate").
//
// One of two ELD adapters behind lib/eld/index.js — the provider-neutral
// selector that server.js consumes. Every module talks to Routemate through
// this file so a future API change ripples through one file. Auth is X-Api-Key
// header; retry/backoff and 15s timeout mirror the Gemini OCR pattern in
// server.js. Returns the provider-neutral shapes the selector's contract
// defines — notably the vehicle-id key is `eld_vehicle_id` and the driver-id
// key is `eld_driver_id` (shared verbatim with lib/eld/apollo.js), so downstream
// stays provider-agnostic. Everything else here is Routemate-specific.
//
...
async function listIftaMileage(creds, paging = {}) {
	...
	const data = (raw && raw.data) || [];
	return data.map((r) => ({
		eld_vehicle_id: r.vehicleId || r.id || "",
