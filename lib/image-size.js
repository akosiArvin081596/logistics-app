...
const LIMITS = Object.freeze({
	...
	// as what their bytes are (servedType()), so only a type that is served. The
	// forms re-encode a photo to a JPEG at most 1200 px on its long edge, a few
	// hundred KB. A photo they cannot re-encode, but whose bytes are a JPEG, PNG
	// or WebP, is sent as the original file, which can pass 16 MP or 2 MiB, as
	// can a request from another client. checkImage() enforces both (413
	// IMAGE_TOO_LARGE), and the form shows that refusal inline.
	TRUCK_PHOTO: freezeLimits([JPEG, PNG, WEBP], 16_000_000, 2 * 1024 * 1024),
});

// The truck photo's byte limit (LIMITS.TRUCK_PHOTO, the one copy of it), and
// the longest base64 data URI that can decode to it, with room for its prefix:
// anything longer is refused before it is decoded at all.
const TRUCK_PHOTO_MAX_BYTES = LIMITS.TRUCK_PHOTO.maxBytes;
