		...
		const fillReport = await page.evaluate((fields) => {
			...
			if (text) {
				for (const [label, value] of Object.entries(text)) {
					...
					if (!el) { missed.push(label); continue; }
					...
					if (shouldEmbedImage) {
						...
						(img.getAttribute("src") ? verified : mismatched).push(label);
						...
						el.value = want;
						(el.value === want ? verified : mismatched).push(label);
					} else {
						el.textContent = want;
						(el.textContent === want ? verified : mismatched).push(label);
					}
				}
			}
