...
async function renderPolicy(docKey, data = {}) {
	...
	try {
		...
		// --- 5a. Clone load + expense rows for service_invoice_owner_op ---
		// Template ships with a single template <tr> in #loads-rows and #expenses-rows;
		// we replace it with one row per item in the data arrays. Empty arrays render
		// a "(none this week)" placeholder row so the table doesn't look broken.
		if (fields.loadsRows || fields.expensesRows) {
			await page.evaluate((data) => {
				...
				if (data.loadsRows !== undefined) {
					fillTbody("loads-rows", data.loadsRows, 3, "(no completed loads this week)");
				}
				if (data.expensesRows !== undefined) {
					fillTbody("expenses-rows", data.expensesRows, 4, "(no fuel or maintenance expenses this week)");
				}
			}, { loadsRows: fields.loadsRows, expensesRows: fields.expensesRows });
		}
