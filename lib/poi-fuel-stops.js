...
async function findFuelStopsAlongRoute(opts = {}) {
	...
	for (const s of byId.values()) {
		...
		ranked.push({
			...
			// Stations with a live price get a boost so a cheap stop isn't dropped
			// by the result cap in favor of a closer stop with no price data.
			score: rankScore(s.primaryType, s.brand) + (s.diesel ? 3 : 0),
		});
	}
