...
// ---------------------------------------------------------------------------
// WHICH LEG IS STILL AHEAD OF THE TRUCK
//
// ⚠️ THE TWIN IS client/src/components/driver/DriverRouteMap.vue:264 (PICKED_UP_RE)
// and the two MUST agree: that file picks which leg to DRAW on the map, this one
// picks which leg to PLAN FUEL for, and a driver comparing the two must not see
// them disagree about where he is in the trip. DriverRouteMap records five
// earlier copies of this predicate drifting apart — keep it at two, and change
// both together. It lives here, not in server.js, so it is unit-testable without
// a server.
//
// Everything NOT matched — Assigned, Dispatched, Heading to Shipper, New,
// Pending — is a truck that still has to reach the shipper, and that run burns
// fuel.
const PICKED_UP_RE = /^(at shipper|loading|in transit|at receiver|unloading)$/i;

/** Has the freight been collected? Unknown/blank status counts as NOT collected,
 *  which is the safe direction: it plans the approach leg in and overstates
 *  rather than understates the fuel needed. */
function isPickedUp(status) {
	return PICKED_UP_RE.test(String(status || "").trim());
}

/**
 * Should the plan include the run to the shipper?
 *
 * Needs all three: a live fix (no truck position -> no approach leg to measure),
 * freight not yet collected (post-pickup the approach is zero BY DEFINITION, and
 * adding one would invent a phantom leg for a truck already loaded), and a
 * geocoded pickup to route to.
 */
function shouldPlanApproach({ fixFresh, pickedUp, hasPickup }) {
	return !!(fixFresh && !pickedUp && hasPickup);
}
