...
// legBurnedGallons(leg) -> gallons actually burned over the leg, or null when the
...
//   - the fill has no positive rise — nothing to calibrate against;
...
function legBurnedGallons(leg) {
	...
	if (!(rise > 0) || !(drop > 0)) return null;
	return gallons * (drop / rise);
}

// receiptDerivedMpg(legs) -> { mpg, miles, gallons, legs, skipped }
...
function receiptDerivedMpg(legs) {
	...
	for (const l of all) {
		...
		const burned = legBurnedGallons(l);
...
// aggregate. Real range at 23% is ~210 mi (median leg). The truck needed
// 15.08 mi per sensor point to finish; it has managed that on 5 of 56 legs.
//
// THE PRIMITIVE IS MILES PER SENSOR POINT, not tank size and not MPG.
// `tank x mpg` multiplies two separately-wrong numbers (#33: tank 300 vs ~198
// measured = 1.51x, mpg 6.5 vs 5.04 receipt-derived = 1.29x, and 1.51 x 1.29 =
// the observed 1.95x). Miles-per-point is measured end-to-end from the ELD
...
// Arrive with fuel still in the tank. Proportional on a long run, floored at 50
// miles on a short one — a fixed 10% would leave 10 miles of margin on a 100-mile
// lane, which is not a margin.
const TRIP_RESERVE_MIN_MI = 50;
const TRIP_RESERVE_FRACTION = 0.10;
// What an UNMEASURED (tank x mpg) range must beat before it may be called a pass.
// Not a safety-factor plucked from the air: it is the overstatement this formula
// was measured to carry on every instrumented truck in the fleet (#33 1.95x,
// #2372 1.97x, #302 2.25x). A truck with no fill history is assumed to be like
// its fleetmates until it proves otherwise.
const ESTIMATED_BASIS_DERATE = 2.0;
...
//   'estimated' — no usable legs; the old tank x mpg point estimate, derated by
//                 the fleet-measured overstatement. low is the ONLY number safe
...
// tripReserveMiles(routeMiles) — fuel we plan NOT to use.
function tripReserveMiles(routeMiles) {
	const r = num(routeMiles);
	if (!isPos(r)) return TRIP_RESERVE_MIN_MI;
	return Math.round(Math.max(TRIP_RESERVE_MIN_MI, r * TRIP_RESERVE_FRACTION));
}
...
//                  basis, or an estimate with 2x of headroom, can earn this.
...
// AND an estimated basis is derated 2x first. 348 vs 346.9 therefore needs
// 2 x (346.9 + 50) = 794 mi before it may say "clears". It says 'insufficient'.
function planTripFuel({ routeMiles, fuelPct, burn, rangeMiles, mpg } = {}) {
	const route = num(routeMiles);
	const interval = rangeInterval({ fuelPct, burn, rangeMiles });
	const reserve = tripReserveMiles(route);
