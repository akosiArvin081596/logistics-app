...
//
// It is a DECLARED SUBSET on purpose, not "every label must match". Measured on
// the shipped templates, vehicle_lease already asks for three labels its HTML
// does not have ("Lessor name", "Lessor address", "Lessor contact") — a
// pre-existing silent drop. A blanket all-must-match rule would refuse every
// vehicle lease on day one; a rule aimed at the signer's identity refuses the
// renders that are actually void. The invoice templates deliberately declare
// none — they are money documents, not signed contracts, and they keep the old
// best-effort behaviour plus the zero-fields backstop.
...
const POLICY_FIELD_MAPS = {
	...
	vehicle_lease(data) {
		...
		return {
			...
			// NOT required: the three "Lessor *" labels above. The shipped
			// vehicle-lease.html has no input for "Lessor name" / "Lessor address" /
			// "Lessor contact", so they are dropped on every render today. That is a
			// real template gap worth closing, but it is LogisX's own constants — not
			// the lessee's identity — so it must not void the lease.
