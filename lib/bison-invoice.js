...
// The returned object also carries a `diagnostics` block so the route can tell
// apart the distinct failure modes (no PDF at all vs no text layer vs OCR threw
// vs read-but-no-order#) and report an accurate, actionable error instead of one
// catch-all message. Top-level fields are unchanged, so existing callers that
// read `.orderNumber` etc. keep working.
async function extractRateConFields(pdfBuffer, opts = {}) {
	const { geminiExtract = null, requireOrderNumber = true } = opts;

	const hadBuffer = Buffer.isBuffer(pdfBuffer) && pdfBuffer.length > 0;
	let fields = { orderNumber: "", poNumber: "", moveNumber: "", trailerNumber: "", totalRate: "" };
	let hadText = false;

	if (hadBuffer) {
		const text = extractPdfText(pdfBuffer);
		// A real text layer yields well over a handful of chars; an image-only
		// PDF inflates to (almost) nothing the label scanner can use.
		hadText = typeof text === "string" && text.trim().length >= 8;
		fields = scanBillingBlock(text);
	}

	const deterministicMatched = Boolean(
		fields.orderNumber ||
			(!requireOrderNumber && (fields.poNumber || fields.moveNumber || fields.trailerNumber)),
	);
	...
	const diag = {
		...
		geminiTried: false,
		geminiReturned: false,
		geminiError: null,
	};

	if (deterministicMatched) return { ...fields, diagnostics: diag };

	// Deterministic scan came up empty. Defer to Gemini only when the route
	// wired it AND we actually have bytes to send — never call OCR on an empty
	// buffer (that path is "no PDF found", not "OCR failed").
	if (hadBuffer && typeof geminiExtract === "function") {
		try {
			const out = await geminiExtract(pdfBuffer);
			if (out && typeof out === "object") {
				return {
					...mergeGeminiFields(fields, out),
					diagnostics: { ...diag, geminiTried: true, geminiReturned: true },
				};
			}
			return { ...fields, diagnostics: { ...diag, geminiTried: true, geminiReturned: false } };
		} catch (err) {
			// Surface as empty fields + a captured reason; the route decides how to fail.
	...
	return { ...fields, diagnostics: diag };
}
