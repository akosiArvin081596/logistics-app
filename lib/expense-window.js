 ...
 *     Completed / POD Received) according to load_status_history. Only the two
 *     status routes write such a row: PUT /api/driver/status (source 'manual' —
 *     the driver's Delivered tap, or dispatch using the same route) and
 *     PUT /api/loads/:loadId/status-override (source 'override'). changed_at is
 *     a button press, not a movement (CLAUDE.md, load-haul.js), which is exactly
 *     the right notion here: "when did this load stop being the driver's work in
 *     progress". It is also the only candidate that is an unambiguous instant —
 *     the sheet's "Status Update Date" / "Completion Date" are zone-less text
 *     with two eras (UTC before 2026-08-03, Houston after) and anyone with the
 *     sheet can edit them. See deliveredAtFromHistory() for why the ENTRY, not
 *     the latest row.
 *
 *   • NO RECORDED DELIVERY TIME → NOT ELIGIBLE ('unknown'). A load marked
 *     delivered any other way — the admin row editors (PUT /api/data/:rowIndex,
 *     PUT /api/load/:loadId) or a direct sheet edit — has no history row, and
 ...
 *     the server gate is Driver-role only. ⚠️ The editors are left out of the
 *     history ON PURPOSE: other readers of the table (the haul replay, the public
 *     tracker's "actual" delivery time) treat its rows as the status routes'
 *     own, and editor rows would shift a deadhead window or publish a wrong
 *     delivery time. Measured 2026-09-23: 0 of 65 loads delivered in the prior
 *     60 days lacked a delivery row, so 'unknown' is rare in practice.
 *
...
// The best of several verdicts for one load id: it takes receipts if ANY of them
// does; otherwise the most informative refusal is reported.
//
// ⚠️ IN PRODUCTION THIS RECEIVES ONE VERDICT. The sheet does hold duplicated ids
// (a rate-con that arrived as two emails in one poll — CLAUDE.md, "A LOAD ARRIVES
// AS TWO EMAILS"), but both callers read getJobTrackingCached(), whose
// deduplicateLoads() keeps only the BOTTOM row per id. So a live row sitting
// above a cancelled "#id" copy never reaches this function: the cancelled copy
// is the load, it is refused here, and the driver app does not list it. The list
// form is defensive — it keeps the gate and the driver app's verdict identical if
// the cache ever stops deduplicating.
