...
// Two tests, because neither alone is sufficient and each catches what the other
// misses:
//   RAW      — a cell whose FIRST byte is dangerous, including a literal tab.
//              Tab is in OWASP's dangerous-lead set (some importers re-parse on
//              it) and was in this guard's original character class.
//   STRIPPED — a formula hiding behind leading whitespace, e.g. " =cmd|…" or
//              "\r=cmd|…", which a raw first-character test walks straight past.
// Testing only the stripped form regressed the raw-tab case: "\tx" strips to
// "x", matches nothing, and went out unguarded. Testing only the raw form lets
// " =cmd|…" through. The union is a superset of both.
const DANGEROUS_RAW = /^[=+\-@\t]/;
const DANGEROUS_STRIPPED = /^[=+\-@]/;

function csvCell(v) {
	let s = String(v ?? "");
	if (DANGEROUS_RAW.test(s) || DANGEROUS_STRIPPED.test(s.replace(LEADING_NOISE, ""))) s = "'" + s;
	return `"${s.replace(/"/g, '""')}"`;
}
