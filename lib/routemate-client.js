...
// an HTTP 500 from here — the known shape, and only while the company endpoint
// still answers — as "list unavailable", NOT a failed sync: it seeds ids from
// telemetry, refreshes each known vehicle through getVehicle() below, and
// reports success. Vehicle discovery is also covered by listLiveLocations()
// (polled every 60 s). Any other status (a 4xx, a 502/503/504) or a network
// failure thrown from here is still a failed sync.
...
// not be null" rather than 404. Callers treat ONLY 400 and 404 as "not a
// Routemate vehicle" and stop asking. Any other error (401/403, 408, 409, 429,
// a 5xx, no answer at all) says nothing about the id and must never be
// remembered as "not ours". request() fails fast on a 4xx other than 429.
