...
//
// A file with no usable createdTime is ranked LAST rather than dropped, so an
// unparseable timestamp does not push a file ahead of better candidates.
// ⚠️ Ranked last still means the cap can cut it: once `cap` in-window files
// exist, undated ones do not make the slice. That is the intended trade (a
// dated near-miss beats an undated unknown), but it is not "always kept" —
// an earlier version of this sentence claimed it was.
// ---------------------------------------------------------------------------
