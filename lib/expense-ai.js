...
//
// An upstream REJECTION is the same user-visible state as an absent key — the
// feature is unavailable and nothing the caller did caused it — so a failure
// carrying .kind === "unavailable" (rejected/restricted key, exhausted quota;
// see lib/gemini-errors.js) also gets a 503 rather than the 502 it used to.
// .kind === "defect" (a request WE built badly) deliberately stays a loud 502.
//
...
async function generate({ systemPrompt, userText, responseSchema, maxOutputTokens }) {
	...
	for (let attempt = 0; attempt <= DEFAULT_RETRIES; attempt++) {
		...
		try {
			...
			if (!resp.ok) {
				// Carries .status / .kind / .reason so the retry loop below and the
				// route above both branch without re-reading the body.
			...
			lastErr = err;
			// Only retry what a second attempt could actually change. A rejected
			// key, an exhausted quota, or a request we malformed fails identically
			// every time, so retrying only multiplies the wait — and on a 429 it
			// deepens the hole it is waiting out. Same fast-fail lib/receipt-ocr.js
			// took on after a 403-ing key left every receipt sitting ~45s.
