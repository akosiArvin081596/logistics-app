...
- **Dispatchers get no broker contact data at all, names included, whatever the cell's format** (owner, 2026-09-26). `sanitizeBrokerColumns()` blanks every `BROKER_WITHHELD_RE` column for every role but Super Admin, JSON contact cells too; the rate-con PDF a Dispatcher can open still carries the broker's contact block.
