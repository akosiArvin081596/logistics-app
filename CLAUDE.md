...
- **Read an image's header before decoding it.** Every in-process decode first calls `checkImage()` (`lib/image-size.js`: known type, readable size, a pixel ceiling) and refuses with 413 `IMAGE_TOO_LARGE` / 415 `UNSUPPORTED_IMAGE_TYPE`; `scripts/test-image-limits.js` pins each guard.
