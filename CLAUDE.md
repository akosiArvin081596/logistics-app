...
- **Anything that moves money ships dormant** — a new flag defaults **off**. All **11** that do, as of 2026-08-25: `INVOICE_AUTOGEN_ENABLED`, `PERIOD_FINALIZE_ENABLED`, `FUEL_GALLONS_RECOVERY_ENABLED`, `MAINTENANCE_NOTICE_ENABLED`, `LINXUP_ENABLED`, `ROUTEMATE_ENABLED`, `SCANKIT_ENABLED`, `RATECON_INDEX_APPLY_ENABLED`, `RATECON_RECONCILE_ENABLED`, `FUEL_EVENTS_ENABLED`, `CHAT_ORPHAN_SWEEP_ENABLED`.
  ...
  - **⚠️ Keep `process.env.` in that grep — the obvious grep answers 16 and the extra one is not a flag.** `grep -rhoE '[A-Z_]+_ENABLED' server.js lib/` returns **16** names against the **15** documented here (11 dormant + 4 kill switches), which reads as "this doc is stale". The 16th is **`STATEMENT_CACHE_ENABLED`** (server.js:42318) — `const STATEMENT_CACHE_ENABLED = !!STATEMENT_TEMPLATE_HASH`, a **derived constant**, not an environment variable. Nothing turns it on or off; it is false only when the statement template hash could not be computed. Verified 2026-08-26: 15 env flags, defaults exactly as tabled above.
...
| Fuel | `fuel_events` |
| Mileage | `eld_miles_daily` — miles per **truck-local** day per driver. ⚠️ Deliberately NOT `routemate_fuel_daily`, which buckets **UTC** days and carries no driver; a UTC-day mileage table disagrees with the Sat–Fri pay week at every seam. `driver_key` is in the UNIQUE key (mid-day handovers make two rows) and `''` is the unattributed sentinel — **never NULL**, which SQLite treats as distinct and would duplicate forever |
