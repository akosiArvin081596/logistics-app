...
**The 4 HIGH are CLOSED as of 2026-08-25** by moving both environments to Node 22 and taking `puppeteer@25` (see the Node table above). `npm audit` now reports **4 on root, all moderate** (client is clean). Do **not** run `npm audit fix --force`.

**⚠️ `puppeteer@25` carries a SILENT breaking change that a smoke test will not catch.** Up to v24 a `Browser` exposed `isConnected()` (method); from v25 it is `connected` (boolean property) and `isConnected` is `undefined`. `lib/pdf-browser.js` only reaches that check when a browser is **already cached**, so the **first** render of a process succeeded and every one after it threw `TypeError: browser.isConnected is not a function` — invoices, onboarding documents and investor statements all broken from the second PDF onward. `browserAlive()` now handles both shapes. Caught by `scripts/test-pdf-cold-start.js`, which renders more than once; the one-render smoke test passed happily. **When bumping puppeteer, render at least twice.**

- **The 4 HIGH all trace to one package: `extract-zip`**, reached only via `puppeteer → @puppeteer/browsers`. **There is no patched version to pin** — the advisory's vulnerable range is `<=2.0.1` and **2.0.1 is the latest**. Puppeteer fixed it by dropping the dependency in 25.x, and **`puppeteer@25` requires `node >=22.12.0` while production runs 20.20.1** — so taking it breaks prod, and raising Node on that box touches ~23 other clients' processes. The last 24.x (24.43.1, which we are on) is still inside the advisory range, so there is no patched minor either. **This is blocked on an infrastructure decision, not a code change.**
  - **Reachability was checked, not assumed:** `extract-zip` runs only when `@puppeteer/browsers` unpacks the Chromium archive it downloads from Google's CDN **at install time**. This app never extracts a zip (the only `decompress` hit is `refresh-env.js` on a **gzip** snapshot, i.e. zlib), it only *creates* them via `archiver` for the receipts bundle, and there is no zip-upload path. Not reachable from user input; exploitation needs a compromised CDN or a MITM during install.
- **The 4 MODERATE** (`gaxios`, `googleapis-common`, `uuid`) need **`googleapis` 128 → 176 — a 48-major jump in the client that IS this app's database.** ⚠️ **Nothing tests it**: the only runner that mentions googleapis (`test-invoice-overrides.js`) merely string-matches `server.js` source and never opens a connection. An untested 48-major bump on the Sheets layer is a worse risk than the moderate advisories it clears.

**Everything semver-safe IS applied** (13 updates, 2026-08-25). `better-sqlite3` sits at 12.11.1, which supports both the VPS's 20.x and a modern local 26.x.

**⚠️ `client/package.json` pins `pdfjs-dist` through an `overrides` block, and it is a security fix, not a preference.** `vue-pdf-embed@2.1.5` (the latest release) depends on `pdfjs-dist ^5.7.284`, which sits inside the **CVE-2026-16633** range (`>=5.6.83 <6.2.108`, arbitrary JS execution from a malicious PDF) and has no 5.x backport — the only forward fix is forcing `6.2.108`. **The override is load-bearing only because `PdfZoomViewer.vue` imports `vue-pdf-embed/dist/index.essential.mjs`**; the package's *default* entry inlines pdf.js and would ignore the override entirely. Changing either half without the other silently reintroduces the CVE. Both files carry the explanation — read them before bumping `vue-pdf-embed`. **⚠️ `pdfjs-dist@6.2.108` also declares `node >=22.13.0 || >=24`, while production and CI both run 20.20.1** — harmless today because Vite bundles it for the BROWSER, so Node's version never applies to it, and CI proves it builds on 20. It becomes real the moment anything imports `pdfjs-dist` server-side. Do not resolve the EBADENGINE warning by raising CI's Node version: that would stop CI matching the box, which is the whole point of the pin.
