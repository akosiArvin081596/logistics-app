...
- `broker-invoice.js` — invoice assembly for **every** broker: `resolveInvoiceTo()` / `resolveBrokerName()` recipient + naming (`resolveInvoiceTo` now prefers the per-load broker billing email — see below), `isBisonLoad()` (now a recipient *branch*, not a gate), deterministic rate-con field extraction, and the payload shape for the Draft Invoice route. See the "Invoice drafting — all brokers" section below.
...
| Finance | `expenses`, `maintenance_fund`, `compliance_fees`, `invoices`, `load_billing`, `investor_config` |
...
- `POST /api/loads/ratecon/extract` — Super Admin/Dispatcher. `{pdfBase64, fileName}` → `{fields, warnings}` via the shared `runRateConGemini()`. **Writes nothing** (no sheet, no DB, no Drive) — it exists so the dispatcher can review before committing. Accepts **PDF and images (PNG/JPG)**: the type is detected from the data-URI prefix / magic bytes (`isValidImageMagic`) and the matching mime is handed to Gemini, which reads images natively; junk payloads are still rejected, size is capped (~15 MB), 503s without `GEMINI_API_KEY`. Rate-limited by `rateConLimiter`.
...
**Billing automation** (Post-Trip Draft Engine — Super Admin/Dispatcher):
- `GET /api/billing/queue` — the "Ready for Billing" queue: loads with a `load_billing` row, each enriched with a checklist (`hasRateCon`, `hasBol`, `hasInvoice`) plus `status`, `draftSubject`, `last_error`. See "Post-Trip Draft Engine + Ready-for-Billing queue" below.
- `POST /api/billing/:loadId/draft` — manually (re)generate a load's invoice + packet + Gmail draft via the shared `assembleAndDraftInvoice()`; covers loads parked as `pending`. Draft-only (never auto-sends).
...
2. **Drag-and-drop (added 2026-07-25, attended).** Dispatcher drops the rate-con — a **PDF or an image (PNG/JPG)** — on the Job Board, Active Loads, or the New Job form. `POST /api/loads/ratecon/extract` parses it (Gemini reads images natively) and **returns without writing**; the review modal shows a **side-by-side document preview** — the original on the left (PDF via `<embed>`, image via `<img>`; stacked below the `md` breakpoint), the editable extracted fields on the right — so the dispatcher can check each field against the source before `POST /api/loads/from-ratecon` replicates the n8n steps server-side. This exists because dispatchers regularly learn about a load from the driver while the app has nothing.
...
- **Recipient** — the draft is addressed **To: the broker's own billing email parsed from the rate-con** (the load's broker AP/billing address), per load. `resolveInvoiceTo()` in `lib/broker-invoice.js` takes it as `brokerBillingEmail` and it **wins outright** (even over Bison). Only when it is absent/blank does the prior fallback apply — `isBisonLoad()` survives *there* as a *branch*, not a gate: Bison → `QPinvoicesUSA@bisontransport.com`; every other broker → `quickpay@megacorplogistics.com`. This **supersedes** the fixed-quickpay recipient shipped earlier (client wants per-broker billing, not a single factoring inbox). Applies to both the manual draft and the auto Post-Trip Draft Engine below.
...
### Post-Trip Draft Engine + Ready-for-Billing queue
Phase-2 automation layered on the invoice drafting above. Where the manual "Draft Invoice Email" button runs on demand, this **auto-triggers on the driver's signed-BOL/POD upload** — a signed BOL *is* the trip-complete signal — so the invoice + supporting packet + Gmail **draft** are staged the moment the paperwork lands. **Draft-only, never auto-sent** (preserves the never-send rule).
- **Shared assembly**: the manual route's core is extracted into `assembleAndDraftInvoice(loadId, { actor, bypassDeliveredGuard })` (the same one-source-of-truth pattern as `generateInvoiceHandler`); both the manual route and the auto-trigger call it, so recipient + packet + invoice numbering stay identical across paths.
- **Trigger**: `POST /api/documents/upload` — inside its existing post-response `setImmediate` block, a POD/BOL upserts `load_billing` → `pending`, then fire-and-forget calls `assembleAndDraftInvoice(loadId, { actor:'auto', bypassDeliveredGuard:true })`. It **never blocks or delays the driver's upload response**.
- **Idempotency + state** live in the new `load_billing` table (`load_id` UNIQUE; `status` `'pending'|'drafted'`, `invoice_number`, `invoice_file`, `draft_mailbox`, `draft_subject`, `drafted_at`, `last_error`, `updated_at`), created with the `CREATE TABLE IF NOT EXISTS` idiom. A load already `drafted` is **never re-drafted and never re-burns an invoice number**; a missing rate-con or other failure leaves `status='pending'` with `last_error` so it surfaces in the queue as "needs attention" rather than failing silently.
- **Recipient**: addressed **To: the broker's parsed billing email** — the same `resolveInvoiceTo()` path as the manual draft (see the Recipient line above).
- **"Ready for Billing" queue** (`GET /api/billing/queue`; the `/billing` view `BillingQueueView.vue`, modeled on `CompletedLoadsTab`): a table/cards of loads with a `load_billing` row showing the `[✓ Rate Con][✓ BOL][✓ Invoice]` checklist (`hasRateCon`/`hasBol`/`hasInvoice`), the draft `status`, and any `last_error`. A **"Review Draft"** button opens Gmail Drafts (`https://mail.google.com/mail/u/0/#drafts`; the draft lives in `info@logisx.com` Drafts, subject shown/copyable) and a **"Generate Draft"** button (`POST /api/billing/:loadId/draft`) covers `pending` rows. No in-app send. The existing manual "Draft Invoice Email" button (CompletedLoadsTab) also writes the `load_billing` record now, so both paths populate the queue.
...
| `/billing` | Super Admin, Dispatcher | "Ready for Billing" queue — rate-con/BOL/invoice checklist; opens staged Gmail drafts |
