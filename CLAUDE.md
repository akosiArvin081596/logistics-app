...
**Role-based data sanitization**: every role but Super Admin is served Job Tracking's broker contact columns (`BROKER_WITHHELD_RE`) blank, by `sanitizeBrokerColumns()` on `/api/dashboard`, `GET`/`PUT /api/load/:loadId` and `GET /api/driver/:driverName` (which also hides the financial columns). `GET /api/data` serves every column as stored, so it is **Super Admin only**. For every caller but a Super Admin, `PUT /api/data/:rowIndex` and `PUT /api/load/:loadId` write each withheld column back exactly as stored, whatever was sent, and refuse a changed cell starting with `=` (400 `FORMULA_NOT_ALLOWED`) — see [`backend-server.md`](docs/claude/backend-server.md), "Broker contact columns".
