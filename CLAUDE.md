...
- `GET /api/poi/fuel-stops?loadId=|originLat=&originLng=&destLat=&destLng=[&limit=]` — diesel truck stops along a load's route (Google Places). Each stop carries `dieselPrice` (live pump price via `fuelOptions`), `priceSource` (`'station'` live | `'regional'` estimate), and `effectivePrice` (what the UI shows). The endpoint **ranks true-cheapest-first** (live prices ascending, then regional-estimate stops by distance) and returns `cheapest` + `livePriceCount`; `regionalDieselPrice` (EIA/fallback) is the per-stop fallback when Google has no price. `poiLimiter`-capped (`fuelOptions` bumps these to the higher Places SKU). Feeds the tracking-map POI layer + cheapest-diesel list.
