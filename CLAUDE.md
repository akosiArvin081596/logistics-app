...
npm run test:unit    # scripts/run-unit-tests.js — 53 standalone runners, ~20 s
...
- **`ci.yml`** — every PR into `main` and every push to `main`. `npm ci` on **both** lockfiles (which is itself the drift test), `node --check server.js`, `npm run test:unit` (the 53 standalone runners, ~20 s), `npm run build:client`, plus an assertion that the build actually emitted a bundle. **Pinned to Node 22.23.2 via `.nvmrc` because that is the runtime pm2 executes both apps with** — see the Node table below. ⚠️ Do **not** re-align this to the box's `/usr/bin/node` (still 20.20.1): that binary belongs to ~23 other tenants, not to this app.
