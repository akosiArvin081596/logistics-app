...
- **Anything that moves money ships dormant** — a new flag defaults **off**. All **11** that do, as of 2026-08-25: `INVOICE_AUTOGEN_ENABLED`, `PERIOD_FINALIZE_ENABLED`, `FUEL_GALLONS_RECOVERY_ENABLED`, `MAINTENANCE_NOTICE_ENABLED`, `LINXUP_ENABLED`, `ROUTEMATE_ENABLED`, `SCANKIT_ENABLED`, `RATECON_INDEX_APPLY_ENABLED`, `RATECON_RECONCILE_ENABLED`, `FUEL_EVENTS_ENABLED`, `CHAT_ORPHAN_SWEEP_ENABLED`.
  - **⚠️ SIX `*_ENABLED` flags deliberately default ON because they are kill switches, not enable switches: `PII_MASK_ENABLED`, `RATECON_EXTRACT_ALERT_ENABLED`, `EXPENSE_DUPLICATE_ALERT_ENABLED`, `FUEL_LOW_ALERT_ENABLED`, `LOAD_HAUL_ELD_ENABLED` and `ELD_STALE_ALERT_ENABLED`.** (`CSRF_HEADER_REQUIRED` is a seventh kill switch under a different naming shape — see the CSRF decision below.) The reasoning is written out at `FUEL_LOW_ALERT_ENABLED`'s definition and applies to all six: *"a safety warning that ships disabled is a safety warning nobody turned on."*
  ...
  - **⚠️ Keep `process.env.` in that grep — the obvious grep answers 18 and the extra one is not a flag.** `grep -rhoE '[A-Z_]+_ENABLED' server.js lib/` returns **18** names against the **17** documented here (11 dormant + 6 kill switches), which reads as "this doc is stale". The 18th is **`STATEMENT_CACHE_ENABLED`** — `const STATEMENT_CACHE_ENABLED = !!STATEMENT_TEMPLATE_HASH`, a **derived constant**, not an environment variable. Nothing turns it on or off; it is false only when the statement template hash could not be computed. Verified 2026-09-19: 17 env flags, defaults exactly as tabled above.
...
npm run check        # node --check server.js — ~49k lines, a syntax error breaks the whole app
npm run test:unit    # scripts/run-unit-tests.js — 59 standalone runners, ~28 s
...
- **`ci.yml`** — every PR into `main` and every push to `main`. `npm ci` on **both** lockfiles (which is itself the drift test), `node --check server.js`, `npm run test:unit` (the 59 standalone runners, ~28 s), `npm run build:client`, plus an assertion that the build actually emitted a bundle. **Pinned to Node 22.23.2 via `.nvmrc` because that is the runtime pm2 executes both apps with** — see the Node table below. ⚠️ Do **not** re-align this to the box's `/usr/bin/node` (still 20.20.1): that binary belongs to ~23 other tenants, not to this app.
- **`deploy.yml`** — push to `main` auto-deploys **staging, then PRODUCTION** (`needs: staging`, so production runs only if staging went green). **⚠️ Production auto-deploys with NO approval gate** (enabled 2026-08-25 at the owner's request). What replaces the human: staging as a hard prerequisite, a smoke check plus a public-edge check, and **automatic rollback** — if production fails verification the workflow returns the box to the SHA it was on, rebuilds and re-verifies (tested for real). The job still goes red; a successful rollback is not a successful deploy. **Staging deliberately does NOT roll back** — it is the canary and a failure should stay put. **To re-arm the gate, add a required reviewer to the `production` Environment; the workflow needs no change.** Manual dispatch still picks one target and any `ref` (that is the rollback path). The remote halves live in `scripts/deploy/*.sh` and are shared by both jobs via `.github/actions/vps-deploy` so they cannot drift.
- **⚠️ Runner→VPS ssh connectivity is intermittently flaky, and with production auto-deploying that means a silent stall.** Measured 2026-08-25: **2 of 25 deploy runs (8%)** failed with `ssh: connect to host … port 22: Connection timed out`. It is **not the box** — fail2ban clear (0 banned), no sshd drops across 97 logins, `MaxStartups` default, no iptables rate limit, load 0.2. It is a network-level SYN blackhole between GitHub's runners and the VPS. The retry helper therefore spans **~5 minutes (5 attempts, 15/30/60/90 s backoff)**, sized from a real outage that exhausted the earlier 3-attempt/70 s budget at 13:19:29 / 13:19:59 / 13:20:39. **A transport failure fails SAFE** — nothing is deployed, production keeps serving the previous SHA — but it fails **silently**. **`deploy-drift.yml` closes that**: every 30 minutes it compares production's HEAD to `origin/main` and resolves to one of four states. `in-sync` passes; **`behind-healable`** (behind but serving 200 — the transport-failure shape) re-runs the deploy **once**; **`behind-already-attempted`** and **`behind-and-unhealthy`** deliberately do NOT heal and fail the job loudly instead. ⚠️ **The one-heal-per-commit marker (`.drift-heal-attempted`, written on the box BEFORE the deploy) is what stops a genuinely broken commit restarting production every 30 minutes.** It is written first on purpose: if the heal dies halfway the marker still records the attempt, so the next tick alarms rather than retries — fail-closed. `behind-and-unhealthy` is treated as an incident, not a missed deploy: healing would paper over it.
...
**Tests & linting:** No Jest/Mocha/Vitest/ESLint configured — but there IS a pipeline now: `npm run ci` locally, and `.github/workflows/ci.yml` on every PR (see CI/CD above). Integration harness at `test-suite.js` — ~150 numbered HTTP tests against a running server. **Do not pin that count here.** It has now drifted twice (the figure read `136` while the file held ~147 `test()` call sites plus ~42 `skip()`, measured 2026-08-26); `test()` and `skip()` both push to `results[]`, so the suite's own printed `results.length` is the only true count and a number written in this file is stale the next time a test is added. Prepare fixtures first, or nearly everything fails at the login on test 2.
...
```
**The other half of the test story: the 59 standalone runners `npm run test:unit` drives.** These are plain `node scripts/test-<thing>.js` — **no server, no fixtures, no sheet override, no rate-limit window** — and each exits 1 on failure. They are the only tests you can run on a whim, and several are explicitly load-bearing elsewhere in this file: `test-db-export-guard.js` (lifts `requireRole` into a bare `new Function`, which is why the CSRF check must stay self-contained), `test-csrf-write-header.js` (pins the two duplicated guard copies identical), `test-invoice-owner-guard.js` (the only authorization coverage on the invoice pdf/submit routes), `scripts/check-ratecon-address-parsing.js` (16 verbatim production addresses locking `cityStateZip()`), plus `test-truck-retirement.js`, `test-investor-expense-scoping.js`, `test-period-lock-queue.js`, `test-payout-carry-freeze.js`, `test-google-timeout-retry.js` (locks the Google API retry-on-timeout behaviour a googleapis major would silently break), `test-eld-feed-staleness.js` (176 assertions on the ELD feed-silence sweep — the alarm on the estimated-pay-basis fallback) and the rest of the money-math set. **Run the ones covering what you touched before reaching for `test-suite.js`** — they are seconds, not a 15-minute window. ⚠️ **`ls scripts/test-*.js` under-reports: it finds 54 of the 59.** The runner globs `scripts/test-*.js`, `scripts/test-*.mjs` **and `scripts/check-*.js`**, so it also picks up the four `scripts/test-*.mjs` client-logic runners and `scripts/check-ratecon-address-parsing.js` — which is the load-bearing one named two sentences up. `node scripts/run-unit-tests.js` prints the authoritative list.

**The suite cannot be run twice inside 15 minutes** — it exhausts its own rate limiters (`exportLimiter` is 20/15min), so a second back-to-back run returns 429 across the export and statement tests and looks like a regression. Wait for the window, or compare against a run from the same cold start. Covers auth, role gating, debug-endpoint auth, webhook secret, chat file validation, canceled-load exclusion. Exits 1 on any failure. When editing `server.js`, run `node --check server.js` before committing (~49k lines; a syntax error breaks the whole app).

## Environment Setup

**⚠️ `.env.example` (587 lines) is the canonical variable list — the block below is a curated subset.** It documents only the vars whose semantics are non-obvious or dangerous. `server.js` reads **80** distinct `process.env.*` names; when wiring anything new, grep `.env.example` first.
...
### Backend (`server.js`)
Single-file Node.js/Express server (**49,918 lines, 240 route registrations** — verified 2026-09-19; the old "~18,450 lines / ~185 endpoints" figure was stale for months, so re-measure with `wc -l` rather than trusting any number written here). Google Sheets is the primary database (Sheets API v4); SQLite for local data; Drive API for uploads; Socket.IO for real-time. Body limit raised to 50mb for large payloads with embedded base64 photos/signatures.
...
**SQLite** (`app.db`, WAL mode) — **61 tables** organized by domain. ⚠️ Regenerate with `grep -oE 'CREATE TABLE IF NOT EXISTS [a-z_]+ *\(' server.js | sed -E 's/.*EXISTS ([a-z_]+).*/\1/' | sort -u | wc -l` — the trailing `\(` is what makes it right. A bare `grep -c` answers **64**, and dropping the paren answers **63** — both read as "this doc is stale". Three comment lines quote the phrase in prose (server.js:11762, 13759, 14109), and only a real `CREATE TABLE` is followed by a column list:
...
| Alerts (dedupe ledgers) | `ratecon_extract_alerts`, `ratecon_reconcile_alerts`, `expense_duplicate_alerts`, `fuel_event_alerts`, `onboarding_doc_alerts`, `eld_feed_alerts` (ELD feed went silent — a **standing condition**, so it carries `resolved_at` and re-opens; see `lib/eld-feed-health.js`) |
| Fuel | `fuel_events` |
...
- `components/` — 16 dirs, feature-organized: `analytics`, `apply`, `dashboard`, `data-manager`, `driver`, `drivers-db`, `financials`, `invest`, `investor`, `investors`, `invoices`, `layout`, `shared`, `trucks`, `ui`, `users`
- `views/` — 33 view components (includes the public `TrackLoadView.vue`)
...
**2. `pdfjs-dist` (client `overrides`) — see the note below.**
...
**Routing** (36 route entries — the 34 user-facing ones below, plus the `/` redirect and the SPA catch-all; all role-guarded):
