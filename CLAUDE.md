...
- **⚠️ `invoices.driver` case is NOT cosmetic — `idx_invoices_driver_week` is a UNIQUE index on `(driver, week_start)` with BINARY collation**, so a mixed-case row sits outside the one-invoice-per-driver-week guarantee. Three readers also compare `driver` exactly: `generateInvoiceNumber()` (server.js ~6861), `PUT /api/invoices/:id/restore` (~11118), and — the live one — the Driver ownership checks on `GET /api/invoices/:id/pdf` and `PUT /api/invoices/:id/submit` (~10815 / ~10844), which do `invoice.driver !== (user.driverName||"").toLowerCase()`, a **one-sided** fold. A driver whose invoice row is mixed-case gets **403 on their own invoice**. Production carries exactly one such row (invoice #40, PAID, inside two locked periods). Fix the readers, not the row.
...
**⚠️ A live Google Maps key sits in plaintext in `Get Distance Matrix`** — a node nothing connects into. Rotate it and delete the node.
...
- **⚠️ A third caller shares this key: n8n.** `Dispatch-v2-fixed.json` → node **"Google Maps Distance"** calls Distance Matrix with `{{ $env.GOOGLE_MAPS_API_KEY }}`, from n8n **Cloud**, whose egress IPs are neither stable nor published. IP-restricting the server key to the VPS therefore breaks email-ingested rate-per-mile **unless** that node is retired first (the app already computes the same thing in `POST /api/loads/from-ratecon` via `calculateRatePerMile()`, and n8n already calls back into the app for extraction). Check whether the node even resolves before planning around it — n8n Cloud blocks `$env` in nodes by default.
- `GET /api/weather` is **dead**: it calls `weather.googleapis.com/v1/forecast:lookup`, which does not exist (the real methods are `currentConditions:lookup` / `forecastDays:lookup`) and returns a 404 HTML page. No client calls it. The Weather API does **not** belong on either allowlist.
