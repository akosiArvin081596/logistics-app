...
- **⚠️ Runner→VPS ssh connectivity is intermittently flaky, and with production auto-deploying that means a silent stall.** Measured 2026-08-25: **2 of 25 deploy runs (8%)** failed with `ssh: connect to host … port 22: Connection timed out`. It is **not the box** — fail2ban clear (0 banned), no sshd drops across 97 logins, `MaxStartups` default, no iptables rate limit, load 0.2. It is a network-level SYN blackhole between GitHub's runners and the VPS. The retry helper therefore spans **~5 minutes (5 attempts, 15/30/60/90 s backoff)**, sized from a real outage that exhausted the earlier 3-attempt/70 s budget at 13:19:29 / 13:19:59 / 13:20:39. **A transport failure fails SAFE** — nothing is deployed, production keeps serving the previous SHA — but production then silently sits behind `main` until someone looks at the Actions tab.
