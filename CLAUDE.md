...
- **Rate-con attachment**: the n8n dispatch workflow (see `Dispatch-v2-fixed.json`, `docs/n8n-webhook-integration-plan.md`) drops every rate-con PDF into a Google Drive folder, named by email subject. `getRateConBytes()` matches on a filename *containing* the load ID and attaches the most recent — needs `drive.readonly`. **The drag-and-drop ingestion path writes there too, named `<loadId>.pdf`** — a load whose rate-con never reaches that folder can never draft an invoice.
