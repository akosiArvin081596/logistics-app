...
- **Auth:** `HOSClientApiKey` — **query param on GETs, body field on POSTs**. ⚠️ Apollo's .NET server does **not** url-decode query values, so a key with any URL-reserved char (notably `?`) can't be delivered on a GET at all — raw truncates and `%3F` is read literally, both → `code:7` "invalid key" **even for a valid key** (verified live 2026-07-25: the `?`-key worked in POST bodies but failed every GET). Only clean `[A-Za-z0-9]` keys work on GET. A boot-time guard warns when `APOLLO_API_KEY` has a reserved char, and `apollo.js` throws `APOLLO_KEY_NOT_URL_SAFE` on GET rather than firing a doomed request.
...
- **Key status (diagnosed 2026-07-25):** the reseller key is **valid + active** (`GetRecap` POST → `code:1` with data) — its earlier `code:7` was purely the `?`-on-GET issue above, not a bad key. **Two things still needed before Apollo goes live:** (1) a **regenerated key with no `?`/symbols** so the GET endpoints (live tracking, drivers) work; (2) **trucks linked as Apollo assets** — the account returns `code:32 HOSASSET_MISSING` (no vehicles provisioned yet). Response-code enum seen live: 1=OK, 3=CLIENT_MISSING, 6=RESELLER_KEY_MISSING, 7=CLIENT_KEY_INVALID, 11=DRIVER_MISSING, 30=INVALID_DATE_RANGE, 32=ASSET_MISSING. Apollo ships dormant (`ELD_PROVIDER=routemate`, `APOLLO_ENABLED=false`) until both are done. **Docs:** `apidocu.reseller.apolloeld.com` (JS-rendered SPA, interactive) + `apidoc.reseller.apolloeld.com` (KB / reference).
