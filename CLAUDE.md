...
| [`ci-deploy.md`](docs/claude/ci-deploy.md) | The CI gate; the push-to-`main` deploy (staging automatically, then production once a reviewer approves, since 2026-09-25); the box deploy lock and exact-SHA deploys; the drift heal; backup freshness; the shared VPS and `ecosystem.config.js`. |
...
- **A guard mounted on a sub-path of a static mount does not hold.** `express.static` decodes and normalizes; a literal-path guard does not. Mount the guard where `express.static` is mounted, do your own decode → fold → collapse → `path.posix.normalize`, match the directory **case-insensitively**, and reject `..` **before** normalize. `normalizedUploadPath()` also refuses any non-ASCII path (every name the app writes is ASCII), and a rule that refuses *on* a row match (the Dispatcher's rate-con rule) matches the file name `COLLATE NOCASE`, because a dev Mac serves a file under any letter case.
...
- **Dispatchers get no broker contact data at all** (owner, 2026-09-26): Job Tracking's contact cells are served blank whatever their format, names and JSON contact cells included, and a non-Super-Admin's save never writes them; `GET` and `POST /api/data` are Super Admin only; rate-con files and invoice drafting are Super Admin only.
- **The investor maintenance-notice popup is dismissed per person, not per tab** (owner, 2026-09-26).
...
**Role-based data sanitization**: every role but Super Admin is served Job Tracking's broker contact columns (`BROKER_WITHHELD_RE`) blank, by `sanitizeBrokerColumns()` on `/api/dashboard`, `GET`/`PUT /api/load/:loadId` and `GET /api/driver/:driverName` (which also hides the financial columns). `GET /api/data` serves every column as stored, so it is **Super Admin only**, and so is `POST /api/data`. For every caller but a Super Admin, `PUT /api/data/:rowIndex` and `PUT /api/load/:loadId` write each withheld column back exactly as stored, whatever was sent, and refuse a changed cell starting with `=` (400 `FORMULA_NOT_ALLOWED`). `POST /api/loads/from-ratecon`, `POST /api/dispatch` and `POST /api/dispatch/reassign` refuse the same value from a non-Super-Admin (`formulaCellRefusal()`, `scripts/test-sheet-formula-doors.js`) — see [`backend-server.md`](docs/claude/backend-server.md), "Broker contact columns".
