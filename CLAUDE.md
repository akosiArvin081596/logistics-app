...
- **The primitive is MILES PER SENSOR POINT, not `tank × mpg`.** That product multiplies two separately-wrong numbers (#33: tank 300 vs ~198 measured = 1.51x; mpg 6.5 vs 5.04 receipt-derived = 1.29x; 1.51 × 1.29 = the observed 1.95x). Miles-per-point is measured end-to-end from the ELD odometer and the gauge between two consecutive `fuel_events`, so it needs neither input and inherits neither error. `burnLegsForVehicle()` builds the legs (server.js, beside `receiptMpgForVehicle`), `burnRateForVehicle()` memoizes 10 min.
- **The measured verdict** (95-day window, production data): the formula overstated on **123 of 125** tank-to-tank legs — #33 56/56 at 1.95x, #2372 64/66 at 1.97x, #302 3/3 at 2.25x. Robust: the aggregate moves only 9.98→12.0 on #33 across every filter combination tried.
...
- **⚠️ The 348-vs-346.9 trap, and why `clears` is judged on the low end.** Once receipt-derived MPG lands, the same truck/route reads **348 mi vs 346.9** — a 1.1-mile margin rendered as a confident number, which reads as *"you'll just make it"* when the honest answer is *"you will not"*. Enabling the feature is what creates that state. Two independent guards: `clears` is decided on `planning` and never on the point estimate, **and** an `estimated` basis is derated by `ESTIMATED_BASIS_DERATE` (2.0 — the measured fleet-wide overstatement, not a made-up safety factor) first. So 348 needs `2 × (346.9 + 50) = 794` mi to claim a pass; it returns `insufficient`. On an estimated basis `typical`/`high` are left **null**, not invented, which lands every non-clearing estimate in `insufficient` rather than the softer `tight`.
