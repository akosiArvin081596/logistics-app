...
- **Signing in to a refreshed copy is opt-in.** `REFRESH_OPERATOR_PASSWORD` — environment only (a password-like argument is refused), 16+ characters — sets **one** existing Super Admin, `REFRESH_OPERATOR_USER` (default `super_admin`). It is applied only where a database is installed, never to the `--sanitize-only` artifact, and the wrappers hand it to `refresh-env.js` and nothing else. **⚠️ Pass it as a one-command prefix (`REFRESH_OPERATOR_PASSWORD="$REFRESH_OPERATOR_PASSWORD" ./scripts/…`), never `export` it:** an exported copy reaches whatever that shell starts next, and a `pm2 restart --update-env` from it would copy it into the running staging process. For `test-suite.js` on a **local** copy, run `scripts/prepare-test-fixtures.js --yes-local-db` after the refresh.
- **Not copied:** `uploads/` (unredacted PII on disk — files 404 in a refreshed environment) and the Google Sheets themselves.
- **`refresh-local.sh` accepts only the options `--help` lists** and refuses anything else by its position, never repeated, before any command runs. The database options reach the preflight, the sanitize on the VPS (quoted for the remote shell, one word each) and the install as one argument vector. **`refresh-staging.sh` installs with the Node pm2 runs `logisx-staging` with**, as the deploy does, and does not restart when `better-sqlite3` cannot open a database under it. Runbook: *Staging installs with the Node pm2 runs it with* in `scripts/README-env-refresh.md`.
