...
- `POST /api/admin/routemate/sync-now` — Super Admin only. 503 when `ROUTEMATE_ENABLED=false` or key unset; else `getCompany()` smoke test, then paginates `listVehicles()` and upserts into `routemate_vehicles`. Logs `audit_trail` action `routemate_sync`. A failed sync answers 502 `ROUTEMATE_SYNC_FAILED`. Since 2026-09-23 `listVehicles()` tries once (`retries: 0`), and no request sleeps after its final attempt. **The vehicle list's HTTP 500 is the known outage, not a failed sync** (since 2026-09-23): when `listVehicles()` answers HTTP 500 while the company endpoint still answers, `routemateSyncVehicles()` sets `listUnavailable`, seeds ids from telemetry, refreshes each vehicle that still has no VIN once through `getVehicle()` (the same refresh runs after a healthy list, where it is usually a no-op), stamps `lastSync` and counts no error — the boot log says `vehicles list unavailable (expected, upstream 500)`, and `sync-now` answers 200 with `listUnavailable: true` and a hint, audited as `routemate_sync`. Anything else — a 502/503/504, a 401/403, a 429, a network error, a failed write, or vehicles the refresh could not fetch — is still a failed sync: 502 (`ROUTEMATE_SYNC_FAILED`, or `ROUTEMATE_REFRESH_FAILED` when the per-vehicle refresh could not fetch every candidate), audited as `routemate_sync_failed`. Per vehicle, only a **400 or 404** from `getVehicle()` means "not a Routemate vehicle" (a Linxup id, say); any other error says nothing about the id and is never remembered as "not ours". `scripts/test-routemate-vehicle-sync.js`.
- `GET /api/routemate/health` — Super Admin only. Returns `{enabled, hasKey, baseUrl, lastSync, lastError, errorsLast24h}`. Since 2026-09-23 it — and `GET /api/eld/linxup/health` — also carries the fleet-wide ELD **feed-health** view from the hourly silence sweep (`lib/eld-feed-health.js`, `eld_feed_alerts`, `ELD_STALE_ALERT_ENABLED` — a kill switch, default ON): `feeds[]` (every feed, each with its `source`), `feedSummary`, `feedThresholds`, `lastFeedSweep` and `lastFeedSweepError`. A stale, never-reported, trickling or orphaned feed alerts once (email to `GMAIL_USER` plus an `eld-feed-silent` dispatch notification); the ledger row resolves when the feed recovers and re-opens if it goes quiet again. A re-opened feed alerts again only once its last alert is 24 h old, so a device hovering at the trickle floor mails at most once a day; `ELD_STALE_ALERT_MAX_PER_DAY` counts **delivered sends** over a rolling 24 h — logged in `server_state` (`eld_feed_alert_sends`), not ledger rows, which a re-open overwrites.
