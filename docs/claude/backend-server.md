...
- **tesseract.js**: OCR on **Receipt** image uploads only (`extractReceiptText()` at `POST /api/documents/upload`, `docType === "Receipt"` — never POD, BOL or Other; this line used to say "POD image uploads", which was never true). It runs **after** the response: `queueReceiptOcr()` reads one receipt at a time (at most 20 queued) and fills `documents.ocr_text` by id, which `GET /api/documents/:loadId` serves to `DocumentList` later — nothing reads the text synchronously, so the upload no longer waits on a Tesseract worker. Once hooked into expense receipts but unwired for low accuracy — that path now uses Gemini (see below). OCR failure never blocks the upload — but **"errors are swallowed" was not true until 2026-09-23, and a try/catch was never what made it so.** tesseract.js 7 rejects a failed job **and then throws from its worker's `message` listener** unless the worker was created with an `errorHandler`; that throw reaches no caller, so it is an uncaught exception and the **server exits**. A crafted 76-byte JPEG that passed the image check did it, after the upload had answered 200. `extractReceiptText()` therefore: creates the worker with `createWorker("eng", 1, { errorHandler })` (never `Tesseract.recognize()`, which passes none); puts a `RECEIPT_OCR_TIMEOUT_MS` (90 s) deadline on **both** the worker start and the recognition — a failed language download leaves `createWorker()` pending forever, which would park the one-at-a-time queue and silently stop all OCR; `terminate()`s the worker in `finally`, and a late-starting one on arrival; and after a start that failed or timed out **pauses OCR for 30 min** (`RECEIPT_OCR_BACKOFF_MS`), because a start that never settles leaves a thread nothing can terminate. ⚠️ **Still open, not reachable by a request:** a failed `initialize` (a corrupt cached `./eng.traineddata`) makes the worker send `reject` **then** `resolve` for one job, and tesseract's listener throws a `TypeError` on the second **with or without** an `errorHandler` — only OCR in a child process closes that. Pinned by `scripts/test-receipt-ocr-crash.js`, which runs tesseract's real listener over a fake transport in a child process.
...
All use `standardHeaders: true` and the `{feature}Limiter` naming convention. **40 are defined** (verified 2026-08-31) — this table covers 12 of them, the ones whose limits are deliberate rather than incidental; `grep -oE 'const [a-zA-Z]+Limiter = ' server.js` for the full list.
