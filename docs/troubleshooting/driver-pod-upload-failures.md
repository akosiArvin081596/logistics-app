...
| `POST /api/documents/upload` (or `POST /api/expenses`) → **`413`** | Payload over nginx `client_max_body_size` (live: `50m` on both long-timeout blocks) or the Express 50 MB limit — or, since 2026-09-23, a JSON **`413 IMAGE_TOO_LARGE`** (or **`415 UNSUPPORTED_IMAGE_TYPE`**: document photos must be JPEG) from the app's own image check (`lib/image-size.js`), which reads the header before anything decodes; the client shows that message as final, with a retake hint | **nginx** body-size (rare; `50m` = Express's 50 MB, so both refuse at the same size) / **frontend** image compression | [nginx-upload-timeouts.md](../runbooks/nginx-upload-timeouts.md) |
