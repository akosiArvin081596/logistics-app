...
- **The backend half is already done.** The POD-column write to Google Sheets is no longer on the upload's critical path: `POST /api/documents/upload` sends the response first and performs the Sheets write in `setImmediate` afterwards. This runbook remains the deploy-time mitigation for slow-uplink bodies, which nginx timeouts still govern.
