  ...
  <div class="p-4">
    <div v-if="fleet.length > 0" class="grid grid-cols-[repeat(auto-fill,minmax(260px,1fr))] gap-3">
      <div v-for="f in fleet" :key="f.Driver"
        class="bg-white/5 border border-white/10 rounded-xl p-4 cursor-pointer hover:border-sky-400/40 transition"
        @click="openDetail(f)">
        <div class="flex justify-between items-start mb-3">
          ...
          <span :class="['text-xs font-semibold px-2 py-0.5 rounded-full', f.Status === 'On Load' ? 'bg-sky-500/10 text-sky-400' : 'bg-emerald-500/10 text-emerald-400']">{{ f.Status }}</span>
        </div>
    ...
    <Teleport to="body">
      <div v-if="selected" class="fixed inset-0 bg-black/40 backdrop-blur-sm z-[200] flex items-center justify-center" @click.self="selected = null">
        <div class="bg-[var(--bg)] rounded-2xl w-[92%] max-w-[480px] max-h-[85vh] flex flex-col shadow-2xl">
          <div class="flex items-center justify-between px-5 py-4 border-b border-white/10">
            <h3 class="text-lg font-bold">{{ selected.Driver || 'Driver' }}</h3>
            <button class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/10 text-gray-400 hover:text-white text-xl" @click="selected = null">&times;</button>
          </div>
          <div class="p-5 overflow-y-auto">
            <div class="mb-4">
              ...
              <div class="bg-white/5 border border-white/10 rounded-lg grid grid-cols-2 overflow-hidden">
                <div class="flex flex-col gap-0.5 p-3 border-b border-white/5">
                  <span class="text-[0.68rem] font-semibold uppercase text-gray-400">Availability</span>
                  <span :class="['text-xs font-semibold px-2 py-0.5 rounded-full w-fit', selected.Status === 'On Load' ? 'bg-sky-500/10 text-sky-400' : 'bg-emerald-500/10 text-emerald-400']">{{ selected.Status }}</span>
                </div>
                <div class="flex flex-col gap-0.5 p-3 border-b border-white/5">
