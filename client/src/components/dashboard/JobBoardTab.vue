<template>
  <div>
    ...
    <div class="overflow-x-auto">
      ...
      <EmptyState v-else-if="filteredJobs.length === 0">{{ needsReviewOnly ? 'No unassigned loads need review.' : (searchQuery ? 'No loads match your search.' : 'All loads are assigned.') }}</EmptyState>
      ...
      <div v-else-if="isMobile" class="mobile-job-list">
        <div v-for="job in paginatedItems" :key="job._rowIndex" class="mobile-job-card" @click="openDetail(job)">
          ...
          <div class="mobile-job-route">
            <div class="mobile-job-row"><span class="mobile-job-label">Pickup</span>
            ...
            <div class="mobile-job-row"><span class="mobile-job-label">Drop-off</span>
              <span class="addr-stack">
...
// Two-line address parts for the synthetic Pickup/Drop-off columns. Line 1 is
// the street (falls back to city/state/zip when the row has no street); line 2
// is city/state/zip, shown only when a distinct street exists on line 1.
