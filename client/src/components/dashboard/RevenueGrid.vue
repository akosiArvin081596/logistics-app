<template>
  <div class="grid grid-cols-3 gap-3 mb-3">
    <div v-for="card in cards" :key="card.key" class="bg-white border border-gray-200 rounded-xl px-4 py-3 flex items-center gap-3 shadow-sm">
      <div class="text-gray-400">{{ card.icon }}</div>
      <div>
        <div class="text-[0.68rem] font-semibold uppercase tracking-wide text-gray-400">{{ card.label }}</div>
        <div class="text-xl font-bold font-mono text-gray-800">{{ card.value }}</div>
      </div>
    </div>
  </div>
</template>
