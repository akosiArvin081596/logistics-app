...
**Vite proxy** (`client/vite.config.js`): `/api`, `/socket.io` (with `ws: true`) and `/uploads` proxy to `VITE_API_TARGET` (default `http://localhost:3000`); the dev port is `VITE_DEV_PORT` (default 5173). Both are read from the repo-root `.env`, by `serve` only: `vite build` never opens that file, because a `NODE_ENV=development` in it (harmless to Express, which only checks for `production`) would turn the build into a development bundle (`scripts/test-vite-build-mode.mjs`).
