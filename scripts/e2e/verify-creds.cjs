...
// The investor and dispatcher logins are optional (none of that role in the DB → none in the creds file).
const logins = [
...
if (!creds.dispatcher) console.log("(the creds file has no dispatcher login — S2a and S3 will SKIP)");
let ok = true;
