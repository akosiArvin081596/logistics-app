...
//   §3 both forms build their pay cells with it, and with nothing else
//   §4 MUTANTS: the amount sent as a number again; the other type's amount
//      sent as "0". Each must fail §1 or §2.
//
...
}
{
  ...
  for (const [file, head, form] of [
    ...
    ok(`§3 ${file}: imports directoryPayCells from lib/driverPay`, /^import \{ directoryPayCells \} from '\.\.\/\.\.\/lib\/driverPay'$/m.test(src))
...
for (const [label, src] of MUTANTS) {
  ...
  const failed = [...cellsSection(m.directoryPayCells), ...serverSection(m.directoryPayCells)].filter((r) => !r.ok)
